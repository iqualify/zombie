...
var Browser = (function (_EventEmitter) {
  ...
  _createClass(Browser, [{
    ...
    value: function destroy() {
      const printUsage = function(label) {
        if (!process.env.PRINT_USAGE) {
          return;
        }
        label = label || '';
        ...
        var heapUsed = formatBytes(process.memoryUsage().heapUsed);
        console.log(`Program is using ${heapUsed} ${label ? '- ' + label : ''}`);
      };

      printUsage('ZOMBIE: before browser.destroy');
      if (this.tabs) {
        this.tabs.closeAll();
        this.tabs = null;
      }
      printUsage('ZOMBIE: after browser.destroy');
      if (global.gc) {
        global.gc();
        printUsage('ZOMBIE: after global.gc');
      }
    }
