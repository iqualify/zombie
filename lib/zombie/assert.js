...
Assert = (function() {
  ...
  Assert.prototype.url = function(expected, message) {
    var absolute, defaultValue, key, results, url, value;
    if (typeof expected === "string") {
      ...
      for (key in expected) {
        ...
        results.push(assertMatch(url[key] || defaultValue, value || defaultValue, message));
      }
      return results;
    }
  };
  ...
  Assert.prototype.attribute = function(selector, name, expected, message) {
    var actual, element, elements, i, len, results;
    ...
    for (i = 0, len = elements.length; i < len; i++) {
      ...
      results.push(assertMatch(actual, expected, message));
    }
    return results;
  };
  ...
  Assert.prototype.hasClass = function(selector, expected, message) {
    var classNames, element, elements, i, len, results;
    elements = this.browser.queryAll(selector);
    assert(elements.length > 0, "Expected selector '" + selector + "' to return one or more elements");
    results = [];
    for (i = 0, len = elements.length; i < len; i++) {
      ...
      results.push(assert(~classNames.indexOf(expected), message || ("Expected element '" + selector + "' to have class " + expected + ", found " + (classNames.join(", ")))));
    }
    return results;
  };
  ...
  Assert.prototype.hasNoClass = function(selector, expected, message) {
    ...
    for (i = 0, len = elements.length; i < len; i++) {
      element = elements[i];
      classNames = element.className.split(/\s+/);
      results.push(assert(classNames.indexOf(expected) === -1, message || ("Expected element '" + selector + "' to not have class " + expected + ", found " + (classNames.join(", ")))));
    }
    return results;
  };
  ...
  Assert.prototype.className = function(selector, expected, message) {
    ...
    for (i = 0, len = elements.length; i < len; i++) {
      ...
      results.push(assertMatch(actual, expected, message || ("Expected element '" + selector + "' to have class " + expected + ", found " + actual)));
    }
    return results;
  };
  ...
  Assert.prototype.style = function(selector, style, expected, message) {
    ...
    for (i = 0, len = elements.length; i < len; i++) {
      ...
      results.push(assertMatch(actual, expected, message || ("Expected element '" + selector + "' to have style " + style + " value of " + expected + ", found " + actual)));
    }
    return results;
  };
  ...
  Assert.prototype.input = function(selector, expected, message) {
    ...
    for (i = 0, len = elements.length; i < len; i++) {
      element = elements[i];
      results.push(assertMatch(element.value, expected, message));
    }
    return results;
  };
