...
module.exports = Cookies = (function(superClass) {
  extend(Cookies, superClass);

  function Cookies() {}

  Cookies.prototype.dump = function() {
    var cookie, i, len, ref, results;
    ref = this.sort(Tough.cookieCompare);
    results = [];
    for (i = 0, len = ref.length; i < len; i++) {
      cookie = ref[i];
      results.push(process.stdout.write(cookie.toString() + "\n"));
    }
    return results;
  };
  ...
  Cookies.prototype.update = function(httpHeader, domain, path) {
    var cookie, cookies, deleteIfExists, i, len;
