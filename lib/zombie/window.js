...
module.exports = function(arg) {
  ...
  browser = arg.browser, params = arg.params, encoding = arg.encoding, history = arg.history, method = arg.method, name = arg.name, opener = arg.opener, parent = arg.parent, referer = arg.referer, url = arg.url;
  name || (name = "");
  url || (url = "about:blank");
  ...
  window._evaluate = function(code, filename) {
    var error, error1, originalInScope, ref, result;
    try {
      ref = [browser._windowInScope, window], originalInScope = ref[0], browser._windowInScope = ref[1];
  ...
  window._destroy = function() {
    var frame, i, j, len, len1, ref, ref1, ws;
    if (closed) {
      return;
    }
    closed = true;
    ref = window._allWebsockets;
  ...
  window._submit = function(arg1) {
    var encoding, method, params, submitTo, target, url;
    url = arg1.url, method = arg1.method, encoding = arg1.encoding, params = arg1.params, target = arg1.target;
    url = HTML.resourceLoader.resolve(document, url);
    target || (target = "_self");
...
loadDocument = function(arg) {
  var browser, document, done, encoding, error, error1, headers, history, method, params, pathname, protocol, ref, url, window;
  document = arg.document, history = arg.history, url = arg.url, method = arg.method, encoding = arg.encoding, params = arg.params;
  ...
  ref = URL.parse(url), protocol = ref.protocol, pathname = ref.pathname;
  switch (protocol) {
    case "about:":
      document.open();
...
Events.EventTarget.prototype.dispatchEvent = function(event) {
  var browser, document, originalInScope, ref, window;
...
HTML.Document.prototype.raise = function(type, message, data) {
  var document, error, i, len, line, partial, ref, window;
