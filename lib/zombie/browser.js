...
Browser = (function(superClass) {
  ...
  function Browser(options) {
    var browser, extension, i, j, len, len1, name, ref;
    if (options == null) {
      options = {};
    }
    browser = this;
    ...
    for (i = 0, len = BROWSER_OPTIONS.length; i < len; i++) {
      name = BROWSER_OPTIONS[i];
    ...
    if (options) {
      ...
      for (k in options) {
        ...
        if (~BROWSER_OPTIONS.indexOf(k)) {
          ref = [this[k], v], restore[k] = ref[0], this[k] = ref[1];
        }
      }
  ...
  Browser.prototype.wait = function(options, callback) {
    var completionFunction, promise, ref, waitDuration;
    if (arguments.length === 1 && typeof options === "function") {
      ref = [options, null], callback = ref[0], options = ref[1];
    }
  ...
  Browser.prototype.visit = function(url, options, callback) {
    var promise, ref, resetOptions, site;
  ...
  Browser.prototype.link = function(selector) {
    var error, error1, i, len, link, ref;
    if (selector instanceof HTML.Element) {
      return selector;
    }
    try {
    ...
    ref = this.querySelectorAll("body a");
  ...
  Browser.prototype.authenticate = function(host, create) {
    ...
    host || (host = "*");
    credentials = (ref = this._credentials) != null ? ref[host] : void 0;
    if (!credentials) {
      if (create) {
  ...
  Browser.prototype.field = function(selector) {
    var error, error1, field, forAttr, i, j, label, len, len1, ref, ref1;
    if (selector instanceof HTML.Element) {
      return selector;
    }
    try {
    ...
    ref = this.queryAll("input[name],textarea[name],select[name]");
    for (i = 0, len = ref.length; i < len; i++) {
      field = ref[i];
  ...
  Browser.prototype._findOption = function(selector, value) {
    var field, i, j, len, len1, len2, m, option, ref, ref1, ref2;
    field = this.field(selector);
  ...
  Browser.prototype.button = function(selector) {
    var button, error, error1, i, input, inputs, j, len, len1, len2, m, ref;
    if (selector instanceof HTML.Element) {
      return selector;
    }
    try {
    ...
    ref = this.querySelectorAll("button");
    for (i = 0, len = ref.length; i < len; i++) {
      button = ref[i];
    ...
    for (m = 0, len2 = inputs.length; m < len2; m++) {
      input = inputs[m];
  ...
  Browser.prototype.saveCookies = function() {
    var cookie, i, len, ref, serialized;
    serialized = ["# Saved on " + (new Date().toISOString())];
    ref = this.cookies.sort(Tough.cookieCompare);
    for (i = 0, len = ref.length; i < len; i++) {
      cookie = ref[i];
  ...
  Browser.prototype.loadCookies = function(serialized) {
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      ...
      results.push(this.cookies.push(Cookie.parse(line)));
    }
    return results;
  };
