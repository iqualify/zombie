...
HTML.HTMLFormElement.prototype.submit = function(button) {
  ...
  process = (function(_this) {
    return function(index) {
      var field, i, len, name, name1, option, ref, ref1, selected, value;
      if (field = _this.elements.item(index)) {
          ...
          if (field.nodeName === "SELECT") {
            ...
            if (field.multiple) {
              ...
              if (selected.length > 0) {
                ...
                value = (ref1 = field.options[0]) != null ? ref1.value : void 0;
              }
        ...
        if (button && button.name) {
          params[name1 = button.name] || (params[name1] = []);
...
HTML.HTMLInputElement.prototype.click = function() {
  var checked, click, i, j, len, len1, original, radio, radios;
  this.focus();
    ...
    case "radio":
      if (!this.getAttribute("readonly")) {
        if (this.checked) {
          ...
          if (!click()) {
            this.checked = false;
            for (j = 0, len1 = radios.length; j < len1; j++) {
              radio = radios[j];
